...
def process_single_item(item, vie_score, max_retries=10000):
    ...
    save_path_fullset_source_image = (
        f"{source_path}/fullset/{group_name}/{instruction_language}/{key}_SRCIMG.png"
    )
    save_path_fullset_result_image = (
        f"{save_path}/fullset/{group_name}/{instruction_language}/{key}.png"
    )
    ...
    for retry in range(max_retries):
        try:
            ...
            pil_image_edited = (
                Image.open(megfile.smart_open(save_path_item, "rb"))
                .convert("RGB")
            ...
            score_list = vie_score.evaluate(
                [pil_image_raw, pil_image_edited], text_prompt
            )
            ...
            if retry < max_retries - 1:
                ...
                print(
                    f"Error processing {save_path_item} (attempt {retry + 1}/{max_retries}): {e}"
                )
                print(f"Waiting {wait_time} seconds before retry...")
                time.sleep(wait_time)
            else:
                print(
                    f"Failed to process {save_path_item} after {max_retries} attempts: {e}"
                )
                return
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--source_path",
        type=str,
        default="/mnt/workspace/lb/Remake/gedit_bench_eval_images",
    )
    ...
    for model_name in evaluate_group:
        ...
        final_csv_path = os.path.join(
            save_path_new, f"{model_name}_combined_gpt_score.csv"
        )
        if megfile.smart_exists(final_csv_path):
            ...
            print(
                f"Loaded {len(processed_samples)} processed samples from existing CSV"
            )
        ...
        for group_name in GROUPS:
            ...
            group_dataset_list = []
            for item in tqdm(
                dataset["train"], desc=f"Processing {model_name} - {group_name}"
            ):
            ...
            group_csv_path = os.path.join(
                save_path_new, f"{model_name}_{group_name}_gpt_score.csv"
            )
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                ...
                for item in group_dataset_list:
                    ...
                    if not megfile.smart_exists(
                        save_path_fullset_result_image
                    ) or not megfile.smart_exists(save_path_fullset_source_image):
                        print(
                            f"Skipping {sample_prefix}: Source or edited image does not exist"
                        )
                        continue
                ...
                for future in tqdm(
                    ...
                    total=len(futures),
                    desc=f"Processing {model_name} - {group_name}",
                ):
        ...
        if not all_csv_list:
            print(
                f"Warning: No results for model {model_name}, skipping combined CSV generation"
            )
            continue
