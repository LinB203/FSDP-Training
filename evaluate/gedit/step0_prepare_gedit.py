...
def main(args):
    ...
    for item in dataset["train"]:
        ...
        target_width, target_height, new_area = calculate_dimensions(
            512 * 512, input_image_raw.width / input_image_raw.height
        )
        resize_input_image = input_image_raw.resize((target_width, target_height))

        save_path_fullset_source_image = os.path.join(
            args.save_path,
            f"fullset/{task_type}/{instruction_language}/{key}_SRCIMG.png",
        )
        save_path_fullset = os.path.join(
            args.save_path, f"fullset/{task_type}/{instruction_language}/{key}.png"
        )
        ...
        instruction_image_paths[key + "_" + item["instruction"]] = {
            "prompt": instruction,
            "id": relative_path,
            "edit_type": task_type,
        }
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Process and save dataset images and instructions."
    )
    parser.add_argument(
        "--save_path",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to save the JSON file with instruction-image mappings.",
    )
