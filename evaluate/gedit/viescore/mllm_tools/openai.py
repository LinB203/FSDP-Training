...
def load_image(
    image: Union[str, Image.Image], format: str = "RGB", size: Optional[Tuple] = None
) -> Image.Image:
    """
...
class GPT4v:
    def __init__(
        ...
        self.url = get_api_key(api_key_path.replace(".env", "_url.env"))
    ...
    def extract_response(self, response):
        ...
        try:
            ...
            if response["error"]["code"] == "content_policy_violation":
                ...
                or response["error"]["code"] == "insufficient_quota"
            ):
